...
ODataServer.prototype.afterInsert = function (fn) {
  if (fn.length === 3) {
    console.warn('Listener function should accept request parameter.')
    var origFn = fn
    fn = function (col, doc, req, cb) {
  ...
  this.cfg.afterInsert = fn.bind(this)
  return this
}
...
ODataServer.prototype.executeInsert = function (col, doc, req, cb) {
  ...
  this.cfg.beforeInsert(col, doc, req, function (err) {
    ...
    self.cfg.insert(col,doc,req,function(err,entity){
      if (!err){
          self.cfg.afterInsert(col,entity,req,function (errAfter){
            if (errAfter){
              return cb(err);
            }
            cb(err,entity);
...
ODataServer.prototype.executeUpdate = function (col, query, update, req, cb) {
  ...
  this.cfg.beforeUpdate(col, query, update, req, function (err) {
    ...
    self.cfg.update(col,query,update,req,function(err,changeCount){
      if (!err){
        self.cfg.afterUpdate(col,query._id,req,function(errAfter){
          if (errAfter){
            return cb(err);
          }
          cb();
...
ODataServer.prototype.afterUpdate = function (fn) {
  if (fn.length === 3) {
    console.warn('Listener function should accept request parameter.')
    var origFn = fn
    fn = function (col, doc, req, cb) {
  ...
  this.cfg.afterUpdate = fn.bind(this)
  return this
}
...
ODataServer.prototype.executeRemove = function (col, query, req, cb) {
  ...
  this.cfg.beforeRemove(col, query, req, function (err) {
    ...
    self.cfg.remove(col,query,req,function(err){
      if (!err){
        self.cfg.afterRemove(col,query._id,req,function(errAfter){
          if (errAfter){
            return cb(err);
          }
          cb();
...
ODataServer.prototype.afterRemove = function (fn) {
  if (fn.length === 3) {
    console.warn('Listener function should accept request parameter.')
    var origFn = fn
    fn = function (col, doc, req, cb) {
  ...
  this.cfg.afterRemove = fn.bind(this)
  return this
}
